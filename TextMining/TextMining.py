...
class Review:
    def __init__(self, review_file_path, opinion_word_file_path, stop_word_file_path, mask_file_path, font_file_path, ws_data_file_path):
        ...
        # font_file_path：用來畫文字雲的字型 otf / ttf 檔
    ...
    def review_csv2dict(self):
        ...
        google_link = df["google_link"].to_list()
        pinyin = df["pinyin"].to_list()
        review_dict = {}
...
while True:
    ...
    score_min = min(all_store_score)
    for i in range(len(all_store_score)):
        all_store_score[i] = all_store_score[i] + (-1 *score_min)
    ...
    # merge store google_link
    all_store_google_link = []
    for ids in review_all.review_dict.keys():
        all_store_google_link.append(review_all.review_dict[ids]["google_link"])

    # merge store name pinyin
    all_store_pinyin = []
    for ids in review_all.review_dict.keys():
        all_store_pinyin.append(review_all.review_dict[ids]["pinyin"])
    # 匯出成 csv 檔
    all_dict = {
    ...
    "Id": all_store_id,
    "Name": all_store_name,
    "pinyin": all_store_pinyin,
    "Score": all_store_score_stdz,
    "IG_link": all_store_IG_link,
    "Google_link": all_store_google_link
    }  
